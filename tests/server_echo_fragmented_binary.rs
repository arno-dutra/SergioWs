...
async fn start_client(client_id: usize) -> Result<()> {
    ...
    w.lock().await.write_message(MessageOut::FragmentedBinary(vec![bytes::Bytes::from_static(b"prefix["), bytes::Bytes::from(u64::try_from(client_id).unwrap().to_be_bytes().to_vec()), bytes::Bytes::from_static(b"]suffix")])).await.unwrap();
...
async fn server_echo_fragmented_binary() -> Result<()> {
